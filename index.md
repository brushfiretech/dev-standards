...
CSS 3.0 can and should be used as long as all functionality degrades gracefully to all targeted browsers so that there may be a degradation in the quality of the experience but the base functionality will still exist.

Inline styles should be avoided in favor of class declarations except when it is not possible or pragmatic to do so.

if/endif statement comment tags should be used to specify different style sheets for different browser situations. These style sheets should be named the same as the main style sheet and append a browser
identifier after a dash (e.g. `style-ie.css`) and should ONLY contain the style declarations that need specific attention for that browser.
...
* All style attributes should be followed by a semi-colon
